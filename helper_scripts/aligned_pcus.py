...
"""
Main function. A target and original word/sentence and a lexicon results in the aligned PCU's of the input words/sentences. 
If target_aligned and original_aligned are given, they should be lists of alignments.
"""
def getAlignedPCUs(target, original, lexicon, target_aligned = None, original_aligned = None):
    target_pcus = []
    original_pcus = []
    target_phones = []

    # Step 1: align target and original, tokenize
    if target_aligned == None or original_aligned == None:
        adaptAlignments = aligner.get_alignments(target, original)
        target_aligned = [x[2] for x in adaptAlignments]
        original_aligned = [x[3] for x in adaptAlignments]


    if target != "":    
        nrTokens = len(target_aligned)

        for tokenIdx in range(nrTokens):
            target_align = target_aligned[tokenIdx]
            original_align = original_aligned[tokenIdx]

            target_phones_list, target_pcu_list, original_pcu_list = computePCUs(target_align, original_align, lexicon)

            target_pcus.append(target_pcu_list[0])
            original_pcus.append(original_pcu_list[0])
            target_phones.append(target_phones_list[0])


    return [target, original, target_pcus, original_pcus, target_phones]
...
def computePCUs(target_align, original_align, lexicon):
    ...
    target = target_align.replace(constants.zero_char, "").lower()
    target_align = target_align
    original_align = original_align
    ...
    try:
        target_graph_align = lexicon.loc[target, "graphemes_align"]
        target_phon_align = lexicon.loc[target, "phonemes_align"]
    except:
        target_graph_align = ["NaN"]
        target_phon_align = ["NaN"]
...
def add_insertion_to_graph_align(graphemes_align, phonemes_align, idx_char):
    ...
    for i in range(len(graphemes_align)):
        ...
        if total_l >= idx_char and i+1 == len(graphemes_align):
            # graphemes_align.insert(i+1, constants.zero_char)
            # phonemes_align.insert(i+1, constants.zero_char)
            ...
            # graphemes_align.pop(i+1)
            # graphemes_align.insert(i+1, "*")
            # phonemes_align.pop(i+1)
            # phonemes_align.insert(i+1, "*")
            before = graphemes_align[: i+1]
    ...
    for span in spans:
        ...
        # If the part is not a pcu, add its pieces to the list and add hyphens to the target lists to compensate for this
            ...
            for i in span_list[1:]:
                ...
                # target_align.insert(index_insert, constants.zero_char)
                # target_phon_align.insert(index_insert, constants.zero_char)
